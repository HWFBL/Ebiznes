...
class ProductRepository @Inject()(dbConfigProvider: DatabaseConfigProvider, categoryRepository: CategoryRepository, ratingRepository: RatingRepository, photoRepository: PhotoRepository)(implicit ec: ExecutionContext) {
  ...
  class ProductTable(tag: Tag) extends Table[Product](tag, "product") {
    ...
    def photo_fk = foreignKey("ph_fk", photo, ph)(_.id)

    def rating_fk = foreignKey("rat_fk", rating, rat)(_.id)
     ...
     */
    def * = (id, name, description, category, price, quantity, rating, photo) <> ((Product.apply _).tupled, Product.unapply)
  ...
  import ratingRepository.RatingTable
   ...
   * id for that person.
   */
  def create(name: String, description: String, category: Int, price: Double, quantity: Int, rating: Long, photo: Long): Future[Product] = db.run {
    // We create a projection of just the name and age columns, since we're not inserting a value for the id column
    (product.map(p => (p.name, p.description, p.category, p.price, p.quantity, p.rating, p.photo))
      // Now define it to return the id, because we want to know what id was generated for the person
      returning product.map(_.id)
      // And we define a transformation for the returned value, which combines our original parameters with the
      // returned id
      into { case ((name, description, category, price, quantity, rating, photo), id) => Product(id, name, description, category, price, quantity, rating, photo) }
      // And finally, insert the product into the database
      ) += (name, description, category, price, quantity, rating, photo)
  }
